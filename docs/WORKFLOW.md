...
| **docs/ARCH.md** | Structure or technology changes (e.g. new build step, new dependency, new deploy target). Aussi lorsque la convention de **cache busting** (`?v=` sur CSS/JS dans `index.html`, `TEMOIGNAGES_JSON_QUERY_BUST` / `REMAINING_SEATS_JSON_QUERY_BUST` pour les JSON) ou les règles **CDN / navigateur** décrites dans ARCH évoluent. |
