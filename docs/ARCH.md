...
| **Malix trade-session** | Encodage/décodage et protocole d’échange local (offre + acceptation bilatérale via QR codes), sans backend | malix/assets/trade-session.js |
...
- **Malix échange:** Protocole local QR-only (scan/copie de code) ; aucun backend ni stockage serveur.
